...
{
    public class DungeonPDrawer : MonoBehaviour
    {
		private static DungeonPLayout CurrentImg
		{
			get { return DungeonPEditorWindow.CurrentLayout; }
			set { DungeonPEditorWindow.CurrentLayout = value; }
		}
		...
		public static void DrawImage(DungeonPLayout img)
		{
		...
		public static Texture2D CalculateBlendedTex(List<DungeonPLayer> _layers)
		{
			if (_layers.Count > 0)
			{
				...
				for (int i = 0; i < _layers.Count; i++)
				{
					...
					if (_result == null)
					{
						_result = DungeonPBlendModes.Blend(_layers[i].tex, _layers[i].Opacity, _layers[i].tex, _layers[i].Opacity, _layers[i].mode);
					}
					else
					{
						_result = DungeonPBlendModes.Blend(_result, 1, _layers[i].tex, _layers[i].Opacity, _layers[i].mode);
					}
				}
				if (_result == null)
				{
		...
		public static void DrawToolbar(Rect window, Vector2 mousePos)
		{
			...
			{
				CurrentImg = DungeonPSession.OpenImage();
				if (CurrentImg == null)
					return;
			}
			if (GUI.Button(new Rect(115, 4, 50, 30), "Export"))
			{
				DungeonPExportWindow.Init(CurrentImg);
			}
			...
			if (CurrentImg.tool == DPTool.ColorPicker)
			{
		...
		public static void DrawLayerPanel(Rect window)
		{
			...
			for (int i = 0; i < CurrentImg.layers.Count; i++)
			{
				...
				DungeonPLayer tempLayer = CurrentImg.layers[i];
				...
				if (tempLayer.enabled != layerEnabled)
					tempLayer.ParentLayout.dirty = true;
			...
			if (GUI.Button(new Rect(12 + 20 * 2, window.height - 20, 18, 18), new GUIContent(Resources.Load("UI/import") as Texture, "Import Image"), smallButon))
			{
				...
				if (path.Length != 0)
				{
					...
					CurrentImg.AddLayer();
					// Set pixel colors
					DungeonPLayer layer = CurrentImg.layers[CurrentImg.layers.Count - 1];
			...
			{
				CurrentImg.layers.Add(new DungeonPLayer(CurrentImg.layers[CurrentImg.selectedLayer]));
			}
			...
			if (GUI.Button(new Rect(12 + 20 * 4, window.height - 20, 18, 18), new GUIContent(Resources.Load("UI/merge") as Texture, "Merge Layer Down"), smallButon))
			{
				if (CurrentImg.selectedLayer > 0)
				{
					DungeonPLayer upper = CurrentImg.layers[CurrentImg.selectedLayer];
					DungeonPLayer lower = CurrentImg.layers[CurrentImg.selectedLayer - 1];
					lower.tex = DungeonPBlendModes.Blend(lower.tex, lower.Opacity, upper.tex, upper.Opacity, upper.mode);
			...
			{
				DungeonPLayerSettings.Init(CurrentImg.layers[CurrentImg.selectedLayer]);
			}
